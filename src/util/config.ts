...
export class ThemeManager {
    ...
    private constructor(
        ...
        private readonly scssPaths: Array<string> = Array.of(
            `${SRC_DIR}/src/style/modules.scss`,
        ),
    ...
    private async allSCSS(): Promise<string[]> {
        ...
        return (await results).map(result => result.match(
            v => v!,
            e => ""
        ));
    }
    ...
    public startMonitors(): void {
        this.monitors = this.monitors ?? [
            this.configPath
        ].map(path => monitorFile(
            path,
            (file: string, event: Gio.FileMonitorEvent): void => Config.defaultHandler(this.syncLoadStyle.bind(this), file, event)
        ));
    }
}
...
export class Config {
    ...
    public static async readConfigFile(path: string): Promise<Readonly<Config>> {
        try {
            const configContent = (await wrapIO(
                Config.logger,
                readFileAsync(path),
