...
export default async function MixerWindow(): Promise<JSX.Element> {
    ...
    const playbackMenu = (
        ...
        >
            <box vertical vexpand valign={Gtk.Align.START} className={"mixer-box"}>
                {bind(
                    Variable.derive(
                        [bind(wireplumb!.audio, "streams"), bind(wireplumb!, "audio")],
                        (streams, ...args: any[]) => {
                            const deviceStreams = streams
                                ...
                                .map(stream => {
                                    return [
                                        ...
                                        <box hexpand={false} vertical vexpand className={"stream-mixer-box"}>
                                            <slider
                            ...
                            return deviceStreams
                                .slice(0, -1)
                                .map(stream => (
                                    <box hexpand vertical vexpand className={"bar-item stream-box"}>
                                        {stream}
                                        <Separator className={"stream-separator"} />
                                    </box>
                                ))
                                .concat([
                                    <box hexpand vertical vexpand className={"bar-item stream-box"}>
    ...
    return (
        <box className={"bar-item mixer-window-box"} hexpand vexpand>
            {playbackMenu}
            <revealer
                transitionType={Gtk.RevealerTransitionType.CROSSFADE}
                transition_duration={100}
                ...
                revealChild={bind(activeMenu).as(menu => menu === ActiveMenu.PLAYBACK)}
            >
                <box className={"bar-item mixer-box"}>
                    <label label={"hel2lo"}></label>
                    <slider className={"mixer-slider"} width_request={200} min={0} max={100} step={1}></slider>
                </box>
            </revealer>
            <revealer
                transitionType={Gtk.RevealerTransitionType.CROSSFADE}
...
export const MixerBadge = (): JSX.Element => {
    const v = Variable.derive(
        [bind(wireplumb!, "defaultSpeaker"), bind(wireplumb!, "defaultMicrophone")],
        (...devices: AstalWp.Endpoint[]) => {
            return devices.map(device =>
                Variable.derive([bind(device, "volume"), bind(device, "mute")], (volume, isMuted) => {
    ...
    return (
        <box className={"bar-item bar-mixer-badge"}>
            <button
                cursor={"pointer"}
                ...
                onButtonPressEvent={MouseEvents.onSecondaryHandler(async () => {
                    ...
                    (
                        await wrapIO(
                            Logger.get(MixerWindow),
                            execAsync(
                                `sh -x -c 'hyprctl dispatch focuswindow address:$(hyprctl clients -j | jq -r ".[] | select(.initialTitle == \\"BaarMixer\\") | .address")'`
                            ),
                            "failed running"
                        )
                    ).match(
