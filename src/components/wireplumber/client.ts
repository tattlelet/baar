...
export class WirePlumberClient {
    ...
    private async registerMetadataManager(): Promise<Wp.ObjectManager> {
        ...
        metadata.add_constraint(
            Wp.ConstraintType.PW_GLOBAL_PROPERTY, "metadata.name",
            Wp.ConstraintVerb.EQUALS, GLib.Variant.new_string("default")
        );
    ...
    private static withTimeout(deferred: DeferredPromise<void>): Promise<boolean> {
        ...
        return withTimeout(WirePlumberClient.OBJECT_MANAGER_TIMEOUT, async () => wrapAsyncResult(deferred.promise))().then(filled => {
            return filled.match(
                withTimeoutOk => withTimeoutOk.match(
                    loadOk => {
    ...
    private static async toLoadedResult<T>(promise: Promise<T>, successMessage: string, failedMessage: string): Promise<[boolean, T | undefined]> {
    ...
    private async loadPlugin(core: Wp.Core, deferred: DeferredPromise<void>, component: string, type: string, provides: string): Promise<void> {
        core.load_component(component, type, null, "default-nodes-api", null, (obj: Wp.Object | null, result: Gio.AsyncResult) => {
            ...
            obj.activate(Wp.PluginFeatures.ENABLED, null, (finishedObj: Wp.Object | null, activationResult: Gio.AsyncResult, data: any | null) => {
                if (finishedObj === null) {
    ...
    private async registerComponents(core: Wp.Core): Promise<Result<void, undefined>> {
        ...
        this.loadPlugin(core, deferredNodesApiLoad, "libwireplumber-module-default-nodes-api", "module", "default-nodes-api");
        this.loadPlugin(core, deferredMixerApiLoad, "libwireplumber-module-mixer-api", "module", "mixer-api");

        const allPluginsLoaded = all((await Promise.all([
        ...
        const [installObjectManagers, _] = await WirePlumberClient.toLoadedResult((async () => {
            core.install_object_manager(metadataManager);
            this.metadataManager = metadataManager;
            core.install_object_manager(objectManager)
            this.objectManager = objectManager;
        })(), "Installed object managers", "Failed to install object managers");
